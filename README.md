...
| `CLAUDE_CODE_OAUTH_TOKEN` | no | server | Claude **subscription** token (created with `claude setup-token`) used by the control-room copilot (`src/lib/copilot/claudeSubscription.ts`) to explain incidents, draft shift reports, and answer NL queries. Unset means those three endpoints return `503 COPILOT_UNAVAILABLE` rather than fabricating a response; expired means the same, with a message naming the fix. Never sent to the browser, never logged — see that file's doc comment. Requires the `claude` CLI on the server. |
| `ANTHROPIC_MODEL` | no | server | Overrides the model id the copilot asks for. Defaults to `claude-sonnet-4-5`. |
| `CLAUDE_CLI_PATH` | no | server | Absolute path to the `claude` binary when it is not on `PATH`. |
| `COPILOT_CLAUDE_TIMEOUT_MS` | no | server | Per-request timeout for a copilot completion. Defaults to `60000`. |

> `ANTHROPIC_API_KEY` is no longer used. The copilot runs on the operator's Claude subscription, and the adapter actively strips `ANTHROPIC_API_KEY` (plus `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL`) from the model subprocess's environment — claude-code gives an API key precedence over subscription credentials, so a stray key would silently move every copilot call onto per-token billing.
