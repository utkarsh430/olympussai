...
**Read the +1,029 figure as the OBJECTIVE being wrong, not the controller being harmful.** That is the single most important thing on this page about this number, because the opposite reading is the intuitive one and it is false. Those same holds are measured to SAVE passengers time: on urban they are worth +4.04% of total passenger time, the guardrail. The objective calls them harmful because its benefit term sees only **1.4%** of the waiting time a hold actually removes - `lambda x d x (d + h_fwd - h_bwd)` is a ONE-STOP marginal estimate of a benefit that accrues over twenty-five downstream stops and to every following bus - so on urban, the corridor where holding demonstrably works, **it reports the wrong sign**. A guard keyed to that number does not decline harmful holds; it declines holds whose benefit the objective cannot see.

Measured, that is exactly what happens. With the check on and occupancy weighting on, the controller stops holding: urban 1,560 holds -> 3 and total passenger time +4.04% -> +0.02%, suburban and inter-city issue ZERO holds, 3/3 seeds worse on every corridor. Occupancy-blind it halves the holds, costs 0.5-2.9 points of excess-wait gain and moves the guardrail by less than its own seed spread with the seeds disagreeing on two of three corridors - no effect. And `terminal_dispatch` is silenced in **18 of 18** runs regardless of the occupancy switch, because the objective charges `w_c` for standing still at the origin against a wait term it cannot see; that is the one lever with no punctuality cost at all, which is the clearest sign the check is measuring the objective's error rather than the action's harm.
...
**The real fix is a MULTI-STOP WAIT TERM.** Not lambda: calibrating it moves the wait term only from 1.4% to 10% of the truth, so the horizon is the whole remaining error. Once the objective's predicted benefit matches a measured one, flip this knob and `COST_OPTIMAL_SELECTION_ENABLED` together and re-run all three corridors both phases across seeds - the argmin of the objective and the sign test on the objective are the same bet, and neither is earned before then. `test/costOptimalOccupancy.test.ts` pins the mechanism, the fix, and the residual.
...
Lambda is NOT the fix and that is now measured rather than assumed: calibrating it moves the wait term from 1.4% of the truth to 10%. The remaining tenfold is the horizon - the wait term `lambda*d*(d + h_fwd - h_bwd)` is a ONE-STOP marginal estimate of a benefit that accrues over the whole downstream route and to every following bus, and urban has twenty-five stops.
