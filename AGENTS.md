...
- **`command_ttl_seconds` (120), `ack_timeout_seconds` (30) and `retry_count` (0) are DEAD COLUMNS** — no code in `control-service/src` reads any of them. Commands really do expire, but `expires_at` is `now() + ttlSeconds` from the CREATE REQUEST, and both consoles that create one hardcode `DEFAULT_TTL_SECONDS = 120` (`ControlRoomCommandForm.tsx`, `console/EngineRecommendationPanel.tsx`). So the 120 s is real and the column with that name is not what produces it. There is no acknowledgement timeout anywhere: an unacked `delivered` command sits until the TTL expires it, which makes the TTL the effective ack deadline. Redelivery is `commandDeliverySweep` sweeping everything resting in `authorized`, not keyed to `retry_count`. Tuning any of the three changes nothing.
- **Nothing ever writes `completed`.** `executing` is inside `commands_one_active_per_vehicle_idx`, so an accepted command holds its vehicle's slot on that unique index until `commandTtlSweep` expires it — the full TTL, whatever the length of the action. A twenty-second hold blocks the bus for 120 s.
- **Therefore `cooldown_seconds` (60) is inert too, at the seeded values.** `mpc/safety.ts` does enforce it, but the unique index already blocks for 120 s, so the cooldown can only refuse an instruction following a driver REFUSAL (`failed` is terminal and frees the slot early). Raising it above the effective TTL is what would give it effect. `minimum_action_seconds` is live code seeded at 0, so it filters nothing either — a tuning gap, not a wiring defect, and the two must not be reported as the same thing.
