...
`pnpm test:coverage` (`vitest run --coverage`, `@vitest/coverage-v8`) is wired in both packages. At root, pass `--coverage.reportOnFailure=true` if you need a report while `theme.test.tsx` / `opsFleetMapTheme.test.tsx` are red (see the jsdom note two paragraphs down) — vitest skips writing the coverage table on a failed run otherwise.

Two root test files, `src/tests/unit/theme.test.tsx` and `src/tests/unit/opsFleetMapTheme.test.tsx` (34 tests across both), fail with `TypeError: Cannot read properties of undefined (reading 'clear')` on `localStorage.clear()` in their own `beforeEach` — a jsdom/vitest-environment issue in this branch's dependency set, unrelated to any application code. Confirmed pre-existing (reproduces on an unmodified checkout); not a skip, so `pnpm test`'s exit code stays non-zero until someone fixes the jsdom setup. Do not "fix" it by touching the tests themselves.
