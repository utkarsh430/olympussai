  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-5xl">
        ...
        <Reveal delay={0.1}>
          <ul className="mt-24 flex flex-wrap items-baseline gap-x-10 gap-y-6" aria-label="Directions of intelligence">
            {TERMS.map((term, i) => {
              const scales = ['text-2xl', 'text-4xl', 'text-3xl', 'text-5xl', 'text-3xl', 'text-4xl'];
              const opacities = ['opacity-45', 'opacity-80', 'opacity-60', 'opacity-100', 'opacity-55', 'opacity-75'];
              return (
                <li
                  key={term}
                  className={`ol-display ${scales[i]} ${opacities[i]} ${i % 3 === 1 ? 'text-ol-gold-light' : 'text-ol-ivory'}`}
                >
                  {term}
                </li>
              );
