...
// Palette (mirrors the `ol-*` tokens; inlined so the SVG stays self-contained).
const LINE_GOLD = 'rgba(214,161,58,0.30)';
const LINE_IVORY = 'rgba(242,238,231,0.20)';
const LINE_FAINT = 'rgba(214,161,58,0.16)';
const NODE = '#f3c86a';
const FOCUS = '#f3c86a';
...
function CrowdDiagram() {
  const dots = [
    [40, 40], [58, 52], [46, 66], [70, 44], [64, 70], [84, 58], [52, 84], [78, 80],
    [150, 30], [168, 46], [158, 62], [182, 38], [176, 66], [196, 52], [206, 74],
    [120, 96], [138, 88], [156, 100], [174, 92], [244, 44], [262, 60], [250, 80],
  ];
  return (
    <Frame>
      {/* High-risk zone */}
      <ellipse cx="172" cy="52" rx="46" ry="34" fill="rgba(214,161,58,0.06)" stroke={LINE_GOLD} strokeWidth="1" strokeDasharray="3 4" />
  ...
  return (
    <Frame>
      ...
      <circle
        ...
        strokeWidth="1"
        opacity="0.55"
        style={{ transformBox: 'fill-box', transformOrigin: 'center', animation: 'ol-node-pulse 2.6s ease-in-out infinite' }}
      />
      <Node cx={20} cy={24} />
      <Node cx={18} cy={104} />
      <Node cx={284} cy={30} />
...
function WildfireDiagram() {
  return (
    <Frame>
      <path d="M96 96 C 70 78, 78 50, 108 44 C 128 40, 150 52, 150 72 C 150 92, 122 108, 96 96 Z" stroke={LINE_GOLD} strokeWidth="1" />
      <path d="M76 104 C 40 78, 52 34, 106 26 C 150 20, 190 46, 188 82 C 186 116, 120 130, 76 104 Z" stroke={LINE_IVORY} strokeWidth="1" opacity="0.7" />
      <path d="M58 112 C 8 78, 26 16, 108 8 C 176 2, 232 40, 226 92" stroke={LINE_FAINT} strokeWidth="1" />
      {/* Drift markers */}
...
function FloodDiagram() {
  return (
    <Frame>
      ...
      <path d="M0 86 C 60 78, 110 92, 170 84 C 220 78, 260 90, 300 84 L 300 132 L 0 132 Z" fill="rgba(214,161,58,0.07)" stroke={LINE_GOLD} strokeWidth="1" />
      <path d="M0 100 C 70 94, 120 106, 300 98" stroke={LINE_GOLD} strokeWidth="1" opacity="0.5" strokeDasharray="2 4" />
...
function TextClassDiagram() {
  ...
  return (
    <Frame>
      ...
      {frags.map((y, i) => (
        <line key={i} x1="16" y1={y} x2={i % 2 ? 52 : 44} y2={y} stroke={LINE_IVORY} strokeWidth="2" />
      ))}
      {/* Feed lines into classifier */}
      {frags.map((y, i) => (
        <path key={`f${i}`} d={`M52 ${y} C 90 ${y}, 120 66, 150 66`} stroke={LINE_FAINT} strokeWidth="1" />
      ))}
      {/* Classifier layer */}
...
function DrivingDiagram() {
  return (
    <Frame>
      ...
      {/* Alternative paths */}
      <path d="M24 122 C 110 110, 150 84, 288 40" stroke={LINE_IVORY} strokeWidth="1" opacity="0.45" strokeDasharray="3 5" />
      <path d="M24 122 C 120 118, 190 70, 292 34" stroke={LINE_IVORY} strokeWidth="1" opacity="0.45" strokeDasharray="3 5" />
      {/* Selected optimized route */}
      <path d="M24 122 C 110 112, 160 56, 290 30" stroke={LINE_GOLD} strokeWidth="1.4" />
      {/* Speed markers */}
      {[[74, 100], [128, 78], [182, 58], [236, 42]].map(([x, y], i) => (
        <line key={i} x1={x!} y1={y! - 5} x2={x!} y2={y! + 5} stroke={NODE} strokeWidth="1" opacity="0.6" />
      ))}
      <Node cx={24} cy={122} />
...
function DeepfakeDiagram() {
  return (
    <Frame>
      {/* Divide */}
      <line x1="150" y1="16" x2="150" y2="116" stroke={LINE_GOLD} strokeWidth="1" strokeDasharray="2 5" />
      {/* Left: coherent contour */}
      <path d="M40 66 C 40 34, 92 26, 116 40 C 132 50, 132 82, 116 92 C 92 106, 40 98, 40 66 Z" stroke={LINE_IVORY} strokeWidth="1" />
      <path d="M40 66 C 70 60, 92 60, 116 66" stroke={LINE_FAINT} strokeWidth="1" />
      {/* Right: fractured contour with inconsistency ticks */}
      <path d="M184 40 C 210 30, 250 34, 260 52 M262 60 C 262 78, 250 92, 236 96 M228 98 C 208 100, 190 92, 186 78" stroke={LINE_IVORY} strokeWidth="1" opacity="0.7" />
      <path d="M184 66 L 200 62 L 214 70 L 230 60 L 246 68 L 260 62" stroke={LINE_GOLD} strokeWidth="1" />
      {/* Inconsistency markers */}
      {[[214, 70], [246, 68], [230, 60]].map(([x, y], i) => (
        <g key={i}>
...
function SpeechDiagram() {
  ...
  return (
    <Frame>
      ...
      <rect x="0" y="34" width="300" height="26" fill="rgba(214,161,58,0.05)" />
      <rect x="0" y="76" width="300" height="20" fill="rgba(242,238,231,0.04)" />
