...
export function CalculationPanel({
  ...
  return (
    <details
      className="group rounded-lg border border-holo-glow/20 bg-void-900/60"
      data-testid="calculation-panel"
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2.5 transition-colors hover:bg-holo-glow/[0.04] [&::-webkit-details-marker]:hidden">
        <span className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.14em] text-holo-glow">
          <Sigma className="h-3.5 w-3.5" aria-hidden />
          View calculations — iteration {withAI.index}
        </span>
        <ChevronDown
          aria-hidden
          className="h-4 w-4 shrink-0 text-holo-glow/50 transition-transform group-open:rotate-180"
        />
      </summary>

      <div className="grid gap-3 border-t border-holo-glow/12 p-3 lg:grid-cols-2">
  ...
  return (
    <div
      className={`rounded border p-2.5 ${
        tone === 'teal'
          ? 'border-holo-teal/25 bg-holo-teal/[0.03]'
          : 'border-alert-crimson/25 bg-alert-crimson/[0.03]'
      }`}
    >
      <h3
        className={`mb-2 font-mono text-[10px] uppercase tracking-[0.16em] ${
          tone === 'teal' ? 'text-holo-teal' : 'text-alert-crimson'
        }`}
      >
        {title}
      </h3>
      ...
      <dl className="space-y-1.5 font-mono text-[10px] leading-relaxed">
        ...
        <div className="border-t border-holo-glow/10 pt-1.5">
          <Line
        ...
        <div className="border-t border-holo-glow/10 pt-1.5">
          ...
          />
          {transition.clamped && (
            <p className="mt-1 text-alert-amber/80">
...
function Line({ term, value }: { term: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:items-baseline sm:gap-2">
      <dt className="shrink-0 text-holo-glow/45">{term}</dt>
      <dd className="min-w-0 break-words text-holo-glow/85">{value}</dd>
    </div>
  );
}
