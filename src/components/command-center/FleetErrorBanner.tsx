...
export function FleetErrorBanner() {
  ...
  const visible = showFixture || showStale || showError || showEmpty;

  const message = showError
    ? `Live UPSRTC feed degraded — ${meta.error}. Showing the last known good data.`
    : showFixture
      ? 'UPSRTC live endpoint unavailable. Operating on sanitized fixture data so the demonstration can continue.'
      : showStale
