...
| "Live data is unavailable — Upstream returned malformed JSON" on `/ops/control-room` | The **external** UPSRTC upstream is down or rate-limiting, not this app | Transient. Retry in a minute; the fixture fallback keeps the UI usable meanwhile. |
